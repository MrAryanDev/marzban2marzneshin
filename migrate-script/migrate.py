...
SCRIPT_COLOR_NAME = (
    "[bold][blue]Marzban[/][/] [yellow]->[/] [bold][red]Marzneshin[/][/]"
)
...
MARZBAN_DOCKER_FILE_ENV_PATH = ["services", "marzban", "environment"]
...
MARZNESHIN_DOCKER_FILE_ENV_PATH = ["services", "marzneshin", "environment"]
...
def check_marzneshin_requirements() -> None:
    ...
    if not exists(MARZNESHIN_DOCKER_FILE):
        error(
            "Marzneshin Docker-Compose(.yml) not found. Please Install The Marzneshin."
        )
...
def get_marzban_transform_protocol() -> str:
    ...
    while True:
        vless_or_vmess = get_input(
            "Which protocol should be the priority for transmission(vless/vmess)"
        ).lower()
...
def export_marzban_data() -> None:
    ...
    transform_protocol = (
        get_marzban_transform_protocol() == "vless"
    )  # True: vless, False: vmess
    ...
    if MARZBAN_SUBSCRIPTIONS_URL_PREFIX_KEY in repository:
        marzban_subscription_url_prefix = repository[
            MARZBAN_SUBSCRIPTIONS_URL_PREFIX_KEY
        ]
    else:
    ...
    def get_user_key(
        user_id: int,
        proxy_type: marzban.ProxyTypes = transform_protocols[transform_protocol],
    ) -> Optional[str]:
        proxy_settings = (
    ...
    def export_system_and_node_usages() -> (
        ...
        node_usages = marzban_session.query(marzban.NodeUsage)

        with Progress(expand=True) as progress:
            for node_usage in progress.track(
                node_usages,
                description="Exporting node-usages",
...
def import_marzban_data() -> None:
    ...
    inbounds = marzneshin_session.query(marzneshin.Inbound).all()
    if not inbounds:
        error("No inbound exists, please create an inbound first.")

    admins = script_session.query(models.Admin)
            ...
            for admin in progress.track(
                ...
                while admin_exists and how_to_deal_with_existing_admins == "rename":
                    ...
                    if last_username_part.isdigit():
                        ...
                        admin.username = (
                            username[: -(len(last_username_part) + 1)] + "_" + number
                        )
                        if len(admin.username) > 32:  # noqa
                            admin.username = (
                                admin.username[: 31 - len(number)] + "_" + number
                            )
                    else:
                        admin.username += "_" + "1"
                        ...
                        .where(
                            marzneshin.Service.id.in_(
                                marzneshin_session.query(
                                    ...
                                    marzneshin.admins_services.c.admin_id
                                    == marzneshin_session.query(  # noqa
                                        marzneshin.Admin.id
                                    )
                                    .where(marzneshin.Admin.username == admin.username)
                                    .scalar_subquery()
                                )
                            )
                        )
                ...
                if not services:
                    ...
                    while service_exists:
                        ...
                        if last_name_part.isdigit():
                            ...
                            service_name = (
                                service_name[: -(len(last_name_part) + 1)]
                                + "_"
                                + str(number)
                            )
                            if len(service_name) > 64:
                                service_name = (
                                    service_name[: 63 - len(str(number))]
                                    + "_"
                                    + str(number)
                                )
                        else:
                ...
                if how_to_deal_with_existing_admins == "update" and admin_exists:
                    final_users = list(
                        ...
                        .where(
                            marzneshin.User.admin_id
                            == marzneshin_session.query(marzneshin.Admin.id)
                            .where(marzneshin.Admin.username == admin.username)
                            .scalar_subquery()
                        )
                        .all()
    ...
    def import_system_and_node_usages() -> None:
        ...
        node_usages = script_session.query(models.NodeUsage)

        with Progress(expand=True) as progress:
            for node_usage in progress.track(
                node_usages,
                description="Importing node-usages",
                total=node_usages.count(),
            ):
                db_node_usage = (
    ...
    try:
        with stream(
            "GET", SOURCE_UPDATER_FILE, follow_redirects=True
        ) as stream_download:
    ...
    warning(
        f"Please Enable And Start {SCRIPT_NAME} service(`systemctl daemon-reload; systemctl enable {SCRIPT_NAME}; systemctl restart {SCRIPT_NAME}`)"
    )
